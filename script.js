/* ================================================
   DENT MATE CLINIC — Premium Animation System
...
(function initHeroTilt() {
  const cards = document.querySelectorAll('.hero__doctor-card');
  if (!cards.length) return;

  cards.forEach(card => {
    const img = card.querySelector('.hero__doctor-img');
    if (!img) return;
    ...
    card.addEventListener('mousemove', e => {
      ...
      // Tilt the entire glass card in 3D space
      card.style.transform = `translateY(-8px) rotateX(${(-dy * 12).toFixed(2)}deg) rotateY(${(dx * 12).toFixed(2)}deg)`;
      // Translate the doctor's portrait inside for premium 3D depth layer
      img.style.transform = `scale(1.12) translateZ(15px)`;
    });
...
if (form) {
  form.addEventListener('submit', function (e) {
    ...
    window.open(`https://wa.me/8801714082916?text=${text}`, '_blank');
...
(function initHeroImagePulse() {
  const images = document.querySelectorAll('.hero__doctor-img');
  images.forEach(img => {
